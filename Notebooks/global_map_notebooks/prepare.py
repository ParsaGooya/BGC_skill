...
def load_ONI(experiment_list: list[Exp],
            ...
            directory: str | Path = '/home/rpg002/BGC_skill/ONI',
            obs_source= 'SODA'):
    ...
    for exp in experiment_list:
        if 'obs' in exp:
            model_exp = 'obs'
            data_path = glob.glob(f'{directory}/{obs_source}*_ONI_*.nc')[0]
            ...
            for model in model_list:
                ...
                data_path = glob.glob(f'{directory}/{_phys_model}_{exp}*_ONI_*.nc')[0]
                ONI_dict[model_exp] = xr.open_dataarray(data_path)
...
def get_corr_ONI(dict_em_data: dict[Var, dict[Exp, state_dict]],
    ...
    for var in dict_em_data:
        ...
        for model_exp in dict_em_data[var]:
            ...
            state_dict.y0 = data_aligned.year.min().values
            state_dict.y1 = data_aligned.year.max().values
